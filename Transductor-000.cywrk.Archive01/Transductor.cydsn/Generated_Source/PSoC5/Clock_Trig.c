...
/*******************************************************************************
* Function Name: Clock_Trig_Start
********************************************************************************
*
* Summary:
...
void Clock_Trig_Start(void) 
{
    ...
    Clock_Trig_CLKEN |= Clock_Trig_CLKEN_MASK;
	Clock_Trig_CLKSTBY |= Clock_Trig_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_Trig_Stop
********************************************************************************
*
* Summary:
...
void Clock_Trig_Stop(void) 
{
    ...
    Clock_Trig_CLKEN &= (uint8)(~Clock_Trig_CLKEN_MASK);
	Clock_Trig_CLKSTBY &= (uint8)(~Clock_Trig_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_Trig_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clock_Trig_CLKEN & Clock_Trig_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clock_Trig_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_Trig_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_Trig__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_Trig_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_Trig_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_Trig_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clock_Trig_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clock_Trig_GetSourceRegister();
    uint16 oldDivider = Clock_Trig_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_Trig_CLKEN & Clock_Trig_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_Trig_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clock_Trig_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clock_Trig_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_Trig_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Trig_MOD_SRC |= modeBitMask & (uint8)Clock_Trig_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Trig_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Trig_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_Trig_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_Trig_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_Trig_MOD_SRC & (uint8)(Clock_Trig_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_Trig_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clock_Trig_GetDividerRegister();
    uint8 oldSrc = Clock_Trig_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_Trig_MOD_SRC |= CYCLK_SSS;
        Clock_Trig_MOD_SRC =
            (Clock_Trig_MOD_SRC & (uint8)(~Clock_Trig_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_Trig_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_Trig_MOD_SRC & Clock_Trig_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_Trig_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Trig_PHASE = clkPhase & Clock_Trig_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Trig_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clock_Trig_PHASE & Clock_Trig_PHASE_MASK;
}
