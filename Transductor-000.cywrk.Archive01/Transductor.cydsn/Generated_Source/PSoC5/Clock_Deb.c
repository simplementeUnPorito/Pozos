...
void Clock_Deb_Start(void) 
{
    ...
    Clock_Deb_CLKEN |= Clock_Deb_CLKEN_MASK;
	Clock_Deb_CLKSTBY |= Clock_Deb_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_Deb_Stop
********************************************************************************
*
* Summary:
...
void Clock_Deb_Stop(void) 
{
    ...
    Clock_Deb_CLKEN &= (uint8)(~Clock_Deb_CLKEN_MASK);
	Clock_Deb_CLKSTBY &= (uint8)(~Clock_Deb_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_Deb_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clock_Deb_CLKEN & Clock_Deb_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clock_Deb_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_Deb_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_Deb__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_Deb_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_Deb_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_Deb_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clock_Deb_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clock_Deb_GetSourceRegister();
    uint16 oldDivider = Clock_Deb_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_Deb_CLKEN & Clock_Deb_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_Deb_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clock_Deb_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clock_Deb_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_Deb_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Deb_MOD_SRC |= modeBitMask & (uint8)Clock_Deb_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Deb_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Deb_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_Deb_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_Deb_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_Deb_MOD_SRC & (uint8)(Clock_Deb_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_Deb_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clock_Deb_GetDividerRegister();
    uint8 oldSrc = Clock_Deb_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_Deb_MOD_SRC |= CYCLK_SSS;
        Clock_Deb_MOD_SRC =
            (Clock_Deb_MOD_SRC & (uint8)(~Clock_Deb_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_Deb_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_Deb_MOD_SRC & Clock_Deb_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_Deb_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Deb_PHASE = clkPhase & Clock_Deb_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Deb_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clock_Deb_PHASE & Clock_Deb_PHASE_MASK;
}
